...
import { LogOut, Plus, Trash2, Star, PackageOpen, AlertTriangle, CheckCircle, Lightbulb, Edit2, X, FileText, Shield, ShoppingCart, ClipboardList, HelpCircle, Link2, ImagePlus } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import ImageUploader from '@/components/ImageUploader';

type Tab = 'packages' | 'reviews' | 'projects' | 'terms' | 'privacy' | 'custom_options' | 'orders' | 'faqs' | 'socials';
...
export default function AdminDashboard({ initialPackages, initialProjects, initialTerms, initialPrivacy, initialCustomOptions, initialOrders, initialFAQs }: { initialPackages: Package[], initialProjects: Project[], initialTerms: TermsData, initialPrivacy: PrivacyData, initialCustomOptions: CustomOption[], initialOrders: Order[], initialFAQs: FAQItem[] }) {
  ...
  useEffect(() => {
    fetch('/api/socials').then(r => r.json()).then(data => {
      if (data.studio) setSocials(data);
    }).catch(() => {});
  }, []);
  ...
  async function handleSaveSocials() {
    ...
    try {
      ...
      if (res.ok) {
        showNotif('success', 'تم حفظ روابط التواصل ✓');
      } else {
        showNotif('error', 'فشل الحفظ');
      }
    } catch {
  ...
  async function handleAddProject(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const imagesText = (formData.get('imagesText') as string) || (formData.get('imagesTextBackup') as string) || '';
    ...
    <div className="min-h-screen bg-background py-24 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex gap-2 mb-8 bg-surface border border-border rounded-2xl p-1.5 w-fit flex-wrap">
          {([['packages', 'الباقات', PackageOpen], ['reviews', 'التقييمات', Star], ['projects', 'المشاريع', Lightbulb], ['terms', 'الشروط', FileText], ['privacy', 'الخصوصية', Shield], ['custom_options', 'خيارات التخصيص', ShoppingCart], ['orders', 'الطلبات', ClipboardList], ['faqs', 'الأسئلة الشائعة', HelpCircle], ['socials', 'روابط التواصل', Link2]] as const).map(([key, label, Icon]) => (
            <button
              key={key}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-2 bg-surface border border-border rounded-3xl p-6 shadow-xl h-fit">
              ...
              <form onSubmit={handleAddPackage} className="space-y-3.5">
                ...
                <div>
                  <label className="block text-xs font-black mb-1 text-foreground/70 uppercase tracking-wide">الصورة الرئيسية</label>
                  <input type="hidden" name="thumbnailUrl" id="pkg-thumbnail-url" />
                  <ImageUploader
                    path="packages/thumbnails"
                    label="ارفع الصورة الرئيسية أو اسحبها هنا"
                    onUpload={(url) => { const el = document.getElementById('pkg-thumbnail-url') as HTMLInputElement; if (el) el.value = url; }}
                  />
                </div>
                <div>
                  ...
                  <textarea name="images" dir="ltr" rows={2} className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm text-left focus:outline-none focus:border-brand-gold transition-colors resize-none" placeholder="أو ارفعها من هنا..." />
                  <ImageUploader
                    multiple
                    path="packages/gallery"
                    label="ارفع صور إضافية"
                    onUpload={(url) => { const el = document.querySelector('[name="images"]') as HTMLTextAreaElement; if (el) el.value = el.value ? el.value + ', ' + url : url; }}
                  />
                </div>
                <div>
            ...
            <div className="lg:col-span-2 bg-surface border border-border rounded-3xl p-6 shadow-xl h-fit">
              ...
              <form onSubmit={handleAddProject} className="space-y-3.5">
                ...
                <div>
                  ...
                  <input type="hidden" name="imagesText" id="proj-images-text" />
                  <ImageUploader
                    multiple
                    path="projects"
                    label="ارفع صور المشروع - أول صورة ستكون الرئيسية"
                    onUploadAll={(urls) => { const el = document.getElementById('proj-images-text') as HTMLInputElement; if (el) el.value = urls.join('\n'); }}
                  />
                  <p className="text-xs text-brand-brown/50 mt-1">
                    أول صورة سيتم اعتباره الصورة الرئيسية للمشروع. أو اكتب الروابط يدوياً أدناه:
                  </p>
                  <textarea 
                    name="imagesTextBackup" 
                    rows={2} 
                    placeholder="أو الصق روابط هنا..."
                    className="w-full bg-background border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-brand-gold transition-colors resize-none ltr text-left mt-2" 
                    dir="ltr"
                  />
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2 bg-surface border border-border rounded-3xl p-6 shadow-xl h-fit">
              ...
              <form onSubmit={handleAddOption} className="space-y-3.5">
                ...
                <div>
                  <label className="block text-xs font-black mb-1 text-foreground/70">صورة الخيار (اختياري)</label>
                  <input type="hidden" value={optionImage} onChange={e => setOptionImage(e.target.value)} />
                  <ImageUploader
                    path="custom-options"
                    label="ارفع صورة للخيار"
                    onUpload={(url) => setOptionImage(url)}
                  />
                  {optionImage && (
                    <div className="mt-2 relative inline-block">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={optionImage} alt="" className="w-20 h-20 rounded-xl object-cover border border-border" />
                      <button onClick={() => setOptionImage('')} className="absolute -top-2 -left-2 bg-red-500 text-white rounded-full p-1"><X size={12} /></button>
                    </div>
                  )}
                </div>
        ...
        {activeTab === 'socials' && (
          <div className="space-y-8">
            <h2 className="text-xl font-black mb-2">روابط التواصل الاجتماعي</h2>
            <p className="text-foreground/60 text-sm">روابط منفصلة لكل فرع. تظهر في صفحة التواصل حسب الفرع النشط.</p>

            {(['studio', 'design'] as const).map(branch => (
              <div key={branch} className="bg-surface border border-border rounded-3xl p-6 shadow-sm">
                <h3 className="text-lg font-black mb-5 flex items-center gap-2 text-brand-brown dark:text-brand-gold">
                  {branch === 'studio' ? '🏢 ستوديو' : '🎨 ديزاين'}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(['discord', 'twitter', 'tiktok', 'email'] as const).map(field => (
                    <div key={field}>
                      <label className="block text-xs font-black mb-1.5 text-foreground/70 uppercase">
                        {field === 'discord' ? 'ديسكورد' : field === 'twitter' ? 'تويتر (X)' : field === 'tiktok' ? 'تيك توك' : 'البريد الإلكتروني'}
                      </label>
                      <input
                        value={socials[branch][field]}
                        onChange={e => setSocials(prev => ({ ...prev, [branch]: { ...prev[branch], [field]: e.target.value } }))}
                        className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-brand-gold transition-colors font-mono text-xs ltr"
                        placeholder={`https://...`}
                      />
                    </div>
                  ))}
            ...
            <button
              ...
              className="w-full bg-brand-gold text-brand-beige font-black py-4 rounded-xl hover:brightness-105 transition disabled:opacity-50 shadow-md text-lg"
            >
