...
import { auth, ensureInit } from '@/lib/firebase';
          ...
          <div className="flex items-center gap-3 flex-shrink-0 ms-auto md:ms-0">
            <Link href="/" className="flex items-center gap-3">
              <div className="flex flex-col items-end leading-none">
                <span className="text-2xl font-black tracking-tight whitespace-nowrap text-foreground">
                  Tuwaiq <span className="text-brand-gold">{branch === 'design' ? 'Design' : 'Studio'}</span>
                </span>
                <span className="text-sm font-bold text-brand-gold/70 tracking-widest mt-[-2px]">طويق</span>
              </div>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center gap-4 flex-shrink-0 me-auto md:me-0">
            ...
            {user ? (
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={() => { ensureInit().then(() => auth?.signOut()); document.cookie = 'admin_auth=; path=/; max-age=0'; }}
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="px-6 pt-2 pb-6 space-y-1">
              ...
              {user ? (
                <button
                  onClick={() => { ensureInit().then(() => auth?.signOut()); setIsMobileMenuOpen(false); }}
