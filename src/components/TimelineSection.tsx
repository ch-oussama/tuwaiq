...
function TimelineNode({ exp, isEven, scrollYProgress, threshold }: NodeProps) {
  ...
  return (
    <>
      ...
      <div className="hidden md:grid relative grid-cols-[1fr_auto_1fr] items-center w-full">
      ...
      <div className="md:hidden flex flex-col items-center gap-3 w-full">
        <motion.div style={{ opacity }}>
          <span className={`text-3xl font-black ${branch === 'studio' ? 'text-[#a78b66]/20' : 'text-[#5c1a16]/25'}`}>
            {exp.year}
          </span>
        </motion.div>
        <motion.div style={{ scale: nodeScale, opacity }}
          className={`w-6 h-6 rounded-full border-[3px] z-30 ${
            branch === 'studio'
              ? 'bg-[#111] border-[#a78b66] shadow-[0_0_18px_rgba(167, 139, 102,0.7)]'
              : 'bg-[#f5ecd8] border-[#5c1a16] shadow-[0_0_18px_rgba(92,26,22,0.85)]'
          }`}
        />
        <motion.div style={{ opacity, x }} className="max-w-xs w-full">
          <div className={`p-5 rounded-2xl border backdrop-blur-sm ${
            branch === 'studio'
              ? 'bg-[#a78b66]/5 border-[#a78b66]/20'
              : 'bg-[#5c1a16]/4 border-[#5c1a16]/15'
...
export default function TimelineSection() {
  ...
  return (
    <section
      ref={containerRef}
      id="experience"
      className="relative py-16 md:py-24 px-4 md:px-6 overflow-visible z-20"
    >
      <div className="max-w-5xl mx-auto relative">
        <div className="text-center mb-16 md:mb-28">
          ...
          <h3 className={`text-3xl md:text-5xl font-black ${branch === 'studio' ? 'text-white' : 'text-[#2d1a12]'}`}>
            محطات صنع الإبداع
          </h3>
        </div>

        <div className="relative flex flex-col gap-14 md:gap-36">
          <div className={`absolute inset-y-0 left-1/2 -translate-x-px w-px md:left-1/2 ${branch === 'studio' ? 'bg-[#a78b66]/20' : 'bg-[#5c1a16]/15'}`} />

          <div className="absolute inset-y-0 left-1/2 -translate-x-px w-px overflow-visible z-10">
            <motion.div
