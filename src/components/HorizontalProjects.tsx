...
export default function HorizontalProjects({ projects }: { projects: Project[] }) {
  ...
  return (
    ...
    >
      <div dir="ltr" className={`sticky top-0 h-screen w-full overflow-hidden flex items-center ${branch === 'studio' ? 'bg-black/80 border-y border-[#a78b66]/20' : 'bg-transparent border-y border-white/10'} relative`}>
            ...
            return (
              <motion.div
                ...
                className={`relative flex-shrink-0 w-[90vw] md:w-[75vw] lg:w-[65vw] max-w-5xl h-[55vh] md:h-[60vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row group ${
                  branch === 'studio' ? 'bg-[#0a0a0a] border border-[#a78b66]/30' : 'bg-black/10 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.08)]'
                }`}
              >
                {/* Number overlay */}
                <motion.div 
                  ...
                  className={`absolute top-4 right-6 text-8xl md:text-9xl font-black pointer-events-none z-0 ${
                    branch === 'studio' ? 'text-white' : 'text-[#2d1a12]/10'
                  }`}
                >
                  {num}
                </motion.div>
                ...
                <motion.div 
                  ...
                  className={`w-full md:w-[55%] h-1/2 md:h-full relative overflow-hidden z-10 ${branch === 'studio' ? 'bg-[#2d1a12]/20' : 'bg-white/20'}`}
                >
                  {(() => {
                  ...
                  <div className={`absolute inset-0 bg-gradient-to-t opacity-80 md:hidden ${
                    branch === 'studio' ? 'from-[#0a0a0a] to-transparent' : 'from-white/20 to-transparent'
                  }`} />
                </motion.div>

                {/* Right Side: Content (Title first, then Description) */}
                <div className={`w-full md:w-[45%] p-8 md:p-12 flex flex-col justify-center z-10 bg-gradient-to-t md:bg-gradient-to-r ${
                    branch === 'studio' ? 'from-[#0a0a0a] to-[#1a1a1a] text-[#f5ecd8]' : 'from-black/15 to-transparent backdrop-blur-md text-[#2d1a12]'
                }`}>
                  ...
                  >
                    <p className={`text-base md:text-lg leading-relaxed mb-8 line-clamp-3 md:line-clamp-4 ${
                      branch === 'studio' ? 'text-[#f5ecd8]/70' : 'text-[#2d1a12]/60'
                    }`}>
                      {project.description}
                    </p>
                    ...
                    <Link href={`/projects/${project.id}`}>
                      <button className={`flex items-center gap-3 px-6 py-3 rounded-full font-bold transition-all hover:gap-4 hover:shadow-lg ${
                        branch === 'studio' 
                          ...
                          : 'bg-[#a78b66]/80 text-[#2d1a12] hover:bg-[#a78b66] backdrop-blur-sm'
                      }`}>
